...
class Config {
        ...
        api.metasync.series(files,
          ...
          () => {
            ...
            impress.nodeId = this.application.isImpress ?
              impress.config.sections.scale.server + 'N' + impress.workerId :
              impress.nodeId;
            if (this.application.isImpress) {
              impress.log.init();
              impress.log.on('open', () => {
  ...
  loadConfigFile(file, callback) {
    const { configDefinition } = impress;
    ...
    impress.createScript(this.application, configFile, (err, exports) => {
      ...
      if (configDefinition.categories.has(sectionName)) {
        validationResult = configDefinition.validate(sectionName, exports);
        if (impress.isMaster) {
  ...
  preprocessConfig() {
    ...
    if (Array.isArray(this.sections.hosts)) {
      if (this.sections.hosts.join('').includes('*')) {
        application.hostsRx = api.common.arrayRegExp(this.sections.hosts);
      }
    } else if (!application.isImpress) {
      impress.log.error('Configuration error: empty or wrong hosts.js');
    }

    const servers = this.sections.servers;
    this.sections.servers = {};
    for (const serverName in servers) {
      ...
      for (let i = 0; i < server.ports.length; i++) {
        ...
        this.sections.servers[serviceName] = srv;
      }
    }

    if (!application.isInitialized && !application.isImpress) {
      application.isInitialized = true;
      const routes = this.sections.routes;
      if (routes) { // Prepare application routes
