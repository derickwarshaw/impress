...
const submodules = [
  'constants', 'stack', 'plugins', 'scheduler', 'scripts', 'workers',
  'files', 'templates', 'config', 'extensions'
];
submodules.forEach(name => require('./' + name));

impress.loadSchemas = callback => {
  const serverConfigSchemas = api.path.join(
    impress.dir, 'schemas', 'config.impress.definition'
  );

  const applicationConfigSchemas = api.path.join(
    impress.dir, 'schemas', 'config.application.definition'
  );

  api.metaschema.fs.loadAndCreate(
    [serverConfigSchemas, applicationConfigSchemas],
    api,
    (err, metaschema) => {
      if (err) {
        console.error(err);
      ...
      impress.configDefinition = metaschema;
      callback();
    },
  );
};
...
impress.start = () => {
  ...
  impress.createSandbox(impress, () => {
    impress.config.loadConfig(() => {
      ...
      const scaleConfig = impress.config.sections.scale;
      if (!impress.isMaster || !scaleConfig || !scaleConfig.check) {
        impress.load();
        return;
      }
      impress.log.system('Startup check: ' + scaleConfig.check);
      api.http.get(scaleConfig.check, res => {
...
impress.load = () => {
  ...
  if (typeof global.gc === 'function' && impress.config.sections.scale.gc > 0) {
    api.timers.setInterval(global.gc, impress.config.sections.scale.gc);
  }
...
impress.loadCertificates = serverConfig => {
  if (serverConfig.key && serverConfig.cert) {
    ...
    try {
      const key = api.fs.readFileSync(certDir + serverConfig.key);
      const cert = api.fs.readFileSync(certDir + serverConfig.cert);
      return { key, cert };
    } catch (e) {
...
impress.dispatcher = (req, res) => {
  ...
  for (const appName in impress.applications) {
    const application = impress.applications[appName];
    const appFound = application.config.sections.hosts &&
      ((application.hostsRx && application.hostsRx.test(host)) ||
      application.config.sections.hosts.includes(host));
    if (appFound) {
